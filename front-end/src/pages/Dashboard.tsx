  ...
  const handlePredict = useCallback(async () => {
    ...
    try {
      ...
      if (predResult.status === 'fulfilled') {
        ...
        // Fall back to deterministic mock so UI is always demonstrable
        ...
        <Card className="bg-slate-900/80 border-slate-800 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
              {/* Symbol */}
              <div>
                <Label htmlFor="symbol" className="text-slate-400 text-xs mb-2 block">
                  Stock Symbol
                </Label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    maxLength={10}
                    className="pl-9 bg-slate-800/50 border-slate-700 text-slate-100 placeholder:text-slate-700 focus-visible:ring-indigo-500/60 focus-visible:border-indigo-500/40 font-mono tracking-wider"
                  />
                </div>
              </div>
              ...
              {/* Start date */}
              <div>
                <Label htmlFor="startDate" className="text-slate-400 text-xs mb-2 block">
                  Start Date
                </Label>
                <Input
                  id="startDate"
                  ...
                  className="bg-slate-800/50 border-slate-700 text-slate-300 focus-visible:ring-indigo-500/60 [color-scheme:dark]"
                />
              </div>

              {/* End date */}
              <div>
                <Label htmlFor="endDate" className="text-slate-400 text-xs mb-2 block">
                  End Date
                </Label>
                <Input
                  id="endDate"
            ...
            <div className="mt-4 flex items-center gap-2 flex-wrap">
              <span className="text-[11px] text-slate-700 uppercase tracking-widest">
                Quick pick
              </span>
              {QUICK_SYMBOLS.map((s) => (
                <button
                  key={s}
                  onClick={() => setSymbol(s)}
                  className="text-xs px-2.5 py-1 rounded-md bg-slate-800 text-slate-400 border border-slate-700 hover:bg-indigo-500/15 hover:text-indigo-400 hover:border-indigo-500/30 transition-all duration-150 font-mono"
                >
                  {s}
                </button>
              ))}
          ...
          >
            <Card className="bg-rose-950/30 border-rose-800/40">
              <CardContent className="p-4 flex items-start gap-3">
                ...
                <div>
                  <p className="text-rose-300 text-sm font-medium">Something went wrong</p>
                  <p className="text-rose-400/70 text-xs mt-0.5">{error}</p>
                </div>
                <button
                  onClick={() => setError(null)}
                  className="ml-auto text-rose-600 hover:text-rose-400 transition-colors"
                >
        ...
        {loading && (
          ...
          >
            ...
            <Card className="bg-slate-900/80 border-slate-800">
              <CardContent className="p-6">
                <div className="animate-pulse h-72 rounded-lg bg-slate-800/50" />
              </CardContent>
            </Card>
          </motion.div>
        )}
        ...
        {!loading && prediction && trendConfig && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
              ...
              {/* Predicted price */}
              <Card className="bg-slate-900/80 border-slate-800 hover:border-slate-700 transition-colors duration-200">
                <CardContent className="p-6">
                  ...
                  {prediction.change_percent != null && (
                    <p
                      className={`text-xs mt-1 flex items-center gap-0.5 ${
                        prediction.change_percent >= 0 ? 'text-emerald-400' : 'text-rose-400'
                      }`}
                    >
      ...
      <AnimatePresence>
        {!loading && !prediction && !error && (
          ...
          >
            ...
            <h3 className="text-slate-400 font-medium mb-2">No prediction yet</h3>
